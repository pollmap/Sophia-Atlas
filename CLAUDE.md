...
| 카테고리 | 현재 (v2.5) | 당초 목표 (v2) | 목표 (v3) |
|----------|-------------|---------------|-----------|
| 철학자 | **289명** ✅ | 187명 (MVP 75) | 222명 |
| 종교 인물 | **220명** ✅ | 156명 (MVP 52) | 200명+ |
| 과학자 | **222명** ✅ | 130명 (MVP 48) | 200명+ |
| 역사/문화 | **288명** ✅ | 100명 (MVP 25) | 150명+ |
| **인물 합계** | **1,019명** ✅ | **573명 (MVP 200)** | **1,000명+** |
| 종교/신화 | **12개** | 15개 | 20개+ |
| 엔터티 (사건·이념 등) | **712개** ✅ | 100개+ | 300개+ |
| 관계 | **8,538개** ✅ | 2,000개+ | 5,000개+ |
...
### Phase 2: 데이터 확장 + 스키마 v2 (완료)
- [x] 통합 타입 시스템 v2.0 (`types/index.ts`)
- [x] CLAUDE.md v2 업데이트
- [x] v1→v2 데이터 마이그레이션 (기존 32명 → Person 형식)
- [x] 철학자 289명 데이터 작성 (목표 187명 초과 달성)
- [x] 종교 인물 220명 데이터 작성 (목표 156명 초과 달성)
- [x] 과학자 222명 데이터 작성 (목표 130명 초과 달성)
- [x] 역사/문화 288명 데이터 작성 (목표 100명 초과 달성)
- [x] 엔터티 712개 작성 (목표 100개 초과 달성, 11종 타입)
- [x] 관계 8,538개 작성 (목표 2,000개 초과 달성)
- [x] 통합 인물 페이지 (`/persons/[id]`)
- [x] 통합 데이터 로더 (`lib/data-loader.ts`)
- [x] 기존 페이지 v2 데이터 연동
- [x] 과학/역사/문화 섹션 페이지 추가
- [x] 통합 타임라인, 세계지도 페이지
- [x] 비교 페이지 (`/compare`)
- [x] 3D 인드라망 시각화 (`/connections`)
- [x] Sophia Type 성향 테스트
- [x] quotes.json v2 스키마 정리
- [x] glossary.json v2 스키마 마이그레이션

### Phase 3: 시각화 강화 (일부 완료)
- [x] D3.js / Three.js Force-directed 3D 그래프 (`/connections`)
- [x] React-Leaflet 실제 세계지도 (`/map`)
- [x] 타임라인에 사건 오버레이 (`/timeline`)
- [x] 인드라망 v2 (Person + Entity 통합)
- [ ] 그래프 성능 최적화 (대규모 노드 렌더링)
- [ ] 지도 클러스터링 개선

### Phase 4: 콘텐츠 심화
- [ ] MDX 상세 페이지 작성 (MVP 200명)
- [x] 동서양 사상 비교 기능 (`/compare`)
- [x] 학습 경로 확장 (`/learn`)
- [ ] 인물별 관계 미니그래프
...
- 데이터 파일 크기 주의: 인물당 ~1KB, 1,019명 ≈ 1MB (분할 로딩 고려)
